...
import DoctorAppointmentDetail from "./DoctorAppointmentDetails";
...
const DoctorDashboard = () => {
  ...
  const todayAppointments = useMemo(
    () =>
      appointments.filter(
        (a) => a.appointmentDetails.appointmentDate === "2025-12-10"
      ),
    [appointments]
  );
    ...
    const loadAssigned = async () => {
      try {
        ...
        const profile = await axios.post(
          "http://localhost:3000/doctor/doctorProfile",
          { email }
        );
        ...
        const patientResp = await axios.post(
          "http://localhost:3000/doctor/get-assigned-patient-details",
          { patientIds }
        );

        setAssignedPatients(patientResp.data.patients || []);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  // ======================
  // PRE-NOTIFICATION / CONSENT HANDLERS
  // ======================
  const handleSendPreNotification = async () => {
    try {
      // Example: backend route that sends Twilio SMS to today's patients
      const todayStr = new Date().toISOString().split("T")[0];

      await axios.post("http://localhost:3000/doctor/send-pre-notification", {
        date: todayStr,
      });

      alert("Pre-notification SMS sent to today's patients.");
    } catch (err) {
      console.error("Error sending pre-notification:", err);
      alert("Failed to send pre-notification. Check console / backend.");
    }
  };

  const handleSendConsentLetter = () => {
    // Abhi ke liye sirf placeholder, baad me PDF / link jaa sakta hai
    alert("Consent letter feature will be implemented soon.");
  };
  ...
  const renderPage = () => {
        ...
        <div className="space-y-10">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Manage follow-up notes and reminders
            </p>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-lg space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <input
                name="appointmentId"
                value={followForm.appointmentId}
                onChange={handleFollowChange}
              ...
              <input
                name="serviceType"
                value={followForm.serviceType}
                onChange={handleFollowChange}
                placeholder="Service Type (therapy/general)"
                className="px-3 py-2 rounded-xl bg-slate-100"
              />
                    ...
                    >
                      <div className="flex justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {fu.followupDate} • {fu.followupTime} •{" "}
                            {fu.serviceType}
                          </p>
        ...
        <div className="space-y-10">
          <div>
            ...
            <p className="text-xl text-slate-500 mt-1">
              Overview of all Ayurveda therapies conducted by the
              practitioner.
            </p>
          </div>
        ...
        <div className="space-y-10">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Manage today's workflow — therapy tasks, consultations, reports &
              more.
            </p>
          </div>

          <div className="flex flex-wrap gap-3">
            {["All", "Pending", "Completed", "Urgent", "Therapy", "Consultation"].map(
              (f) => (
                <button
                  key={f}
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg p-8 flex gap-8 items-center">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 leading-relaxed max-w-xl">
                Expert in Panchakarma with a focus on chronic pain
                management, detox therapies, and holistic healing.
                Practicing at Himalayan Bliss Ayurveda Retreat for 5+
                years.
              </p>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg p-8">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              <p>
                <strong>Center Name:</strong> Himalayan Bliss Ayurveda
                Retreat
              </p>
              <p>
                <strong>Type:</strong> Government · Certified Center
              </p>
              <p>
                <strong>Rating:</strong> 4.5 ★
              </p>
              <p>
                <strong>City:</strong> Dehradun
              </p>
              <p>
                <strong>Address:</strong> Rajpur Road, Near Pacific Hills,
                Dehradun – 248001
              </p>
              <p>
                <strong>Timings:</strong> Morning 09:00–02:00 · Evening
                15:00–21:00
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg p-6">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              <p>
                <strong>Email:</strong> dr.ravi@ayursutra.com
              </p>
              <p>
    ...
    return (
      <>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mt-8">
          <div className="bg-white/90 backdrop-blur-sm p-6 rounded-2xl shadow-md hover:shadow-lg transition">
            ...
            <p className="text-lg font-semibold mt-2">
              {nextAppointment.patientDetails.name}
            </p>
            <p className="text-xs text-slate-500">
        ...
        {/* APPOINTMENTS TABLE + ACTIONS CARD */}
        <section className="mt-8 grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <tbody>
                {appointments.map((a) => (
                  ...
                  >
                    ...
                    <td className="py-3">
                      {formatDate(a.appointmentDetails.appointmentDate)}
                    </td>
                    <td className="py-3">
                      {a.appointmentDetails.appointmentSlot}
                    </td>
                    <td className="py-3">
                      {a.appointmentDetails.ServiceType}
                    </td>
                    <td className="py-3">
                      {a.appointmentDetails.tokenNumber}
                    </td>
                    <td className="py-3">
                      <span
          ...
          <div className="bg-white/95 backdrop-blur p-6 rounded-3xl shadow-lg flex flex-col justify-between">
            <div>
              <h3 className="font-semibold text-sm mb-2">
                Appointment Actions
              </h3>
              <p className="text-xs text-slate-500 mb-4">
                Quickly send SMS notifications to today&apos;s patients.
              </p>
            </div>
            ...
            <div className="space-y-3 mt-2">
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition"
              >
                Send Pre-Notification SMS
              </button>

              <button
                onClick={handleSendConsentLetter}
                className="w-full py-2.5 rounded-xl bg-slate-100 text-slate-800 text-sm font-medium hover:bg-slate-200 transition"
              >
                Send Consent Letter
              </button>
            </div>
            ...
            <p className="text-[11px] text-slate-400 mt-4">
              * Pre-notification SMS will be sent via Twilio to today&apos;s
              scheduled patients once you configure the backend route.
            </p>
          </div>
        </section>
